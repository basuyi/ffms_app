...
function zy_con(id,url,x,y,name)
{
	var s=window.getComputedStyle($$(id),null);
	uexWindow.openPopover(name?name:id,"0",url,"",int(x),int(y),int(s.width),int(s.height),int(s.fontSize),"0");
}
function zy_resize(id,x,y,name)
{
	var s=window.getComputedStyle($$(id),null);
